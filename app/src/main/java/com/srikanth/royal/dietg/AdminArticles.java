...
public class AdminArticles extends AppCompatActivity {
...
Sref= FirebaseStorage.getInstance().getReference().child("Image");
    ...
    private void Storage() {
        ...
        uploadTask.addOnFailureListener(new OnFailureListener() {
            ...
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(AdminArticles.this, ""+e.getMessage(), Toast.LENGTH_SHORT).show();
                loadingbar.dismiss();
            }
        }).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            @Override
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                Toast.makeText(AdminArticles.this, "image upload succesfull", Toast.LENGTH_SHORT).show();
                ...
                Task<Uri> uriTask=uploadTask.continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                    ...
                    public void onComplete(@NonNull Task<Uri> task) {
                        ...
                        if (task.isSuccessful())
                        {
                            ...
                            Toast.makeText(AdminArticles.this, "image url is succesfully got", Toast.LENGTH_SHORT).show();
        ...
        Iref.child(randomkey).updateChildren(itemmap)
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful())
                        {loadingbar.dismiss();
                            Toast.makeText(AdminArticles.this, "item is added succesfull", Toast.LENGTH_SHORT).show();
                            Intent intent= new Intent(AdminArticles.this,category.class);
                            startActivity(intent);
                        }
                        else {
                            loadingbar.dismiss();
                            Toast.makeText(AdminArticles.this, ""+task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                        }
                    }
                });
    }
